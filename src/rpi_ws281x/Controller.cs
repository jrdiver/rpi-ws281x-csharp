...
public class Controller
{
	internal Controller(int ledCount, Pin pin, byte  brightness, bool invert, StripType stripType, ControllerType controllerType)
	{
		...
		LEDColors = Enumerable.Range(0, ledCount).Select(_ => new LED()).ToList();
	}
	...
	private List<LED> LEDColors { get; set; }
	...
	public int LEDCount {
		get => LEDColors.Count;
		set {
			LEDColors = Enumerable.Range(0, value).Select(_ => new LED()).ToList();
			IsDirty = true;
		}
	}
